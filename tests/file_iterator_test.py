...
from src.commun import batch_iterator
...
def test_one_channel():
    results = list(
        batch_iterator("examples", "mouse_A", ieg_channel=True, dapi_channel=False)
    )
...
def test_two_channels():
    results = list(
        batch_iterator("examples", "mouse_B", ieg_channel=True, dapi_channel=True)
    )
    assert len(results[0]) == 4
    roi, img_data, area_name, mouse_directory = results[0]
    assert img_data["ieg"] is not None
